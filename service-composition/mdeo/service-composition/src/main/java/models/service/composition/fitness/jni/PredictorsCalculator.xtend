...
//      predictors.put("loadSmall", object.countLoadNodes(3));
	...
	def EObject loadPlatformResource(File modelFile) {

		var protocol = this.getClass().getResource("").getProtocol();

		if (Objects.equals(protocol, "jar")) {

			val resourceSet = new ResourceSetImpl()
			val resource = resourceSet.createResource(URI.createURI("jar:" + modelFile.path))
			resource.load(Collections.EMPTY_MAP)

			return resource.allContents.head
		}

		val resourceSet = new HenshinResourceSet(modelFile.parent)
		val resource = resourceSet.createResource(URI.createURI(modelFile.name))
		resource.load(Collections.EMPTY_MAP)
	...
	def int calculateOrchestrators(EObject model) {
		...
		return concretePlans.fold(0) [ orchestratorsCount, concretePlan |
			orchestratorsCount + ((concretePlan.getFeature("orchestrators") as List<EObject>).filter [ orchestrator |
				orchestrator.getFeature("concreteServices") !== null
			]).toList.size
		]
	}
	...
	def int calculateHops(EObject model, EObject predictors) {
		...
		orchestrators.forEach [ orchestrator |
			...
			val orchestratorHopsCount = concreteServices.fold(0) [ planHops, concreteService |

				planHops + lookupConnection(
					orchestrator.getFeatureObject("deployedOn").getFeatureInt("ID"),
					concreteService.getFeatureObject("providedBy").getFeatureInt("ID"),
					predictors
				)
			]
	...
	def EObject lookupNode(int id, EObject predictors) {
		...
		return predictors.getFeatureList("nodes").filter [ node |
			node.getFeatureInt("ID") === id
		].head
	}
	...
	def int lookupConnection(int startNode, int endNode, EObject predictors) {
		...
		var hops = connections.filter [ connection |
	...
	def int countTypeNodes(int nodeType, EObject model, EObject predictors) {
		...
		return orchestrators.filter [ orchestrator |
			...
			counter + 1 + concreteServices.filter [ concreteService |
